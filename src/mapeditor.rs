use crate::prelude::*;
use rusterix::{D2PreviewBuilder, GridShader, Rasterizer, Scene, Shader};
use vek::Vec2;
...
impl MapEditor {
    ...
    pub fn draw(&mut self, ui: &mut TheUI, project: &Project) {
        if let Some(render_view) = ui.get_render_view("EditorView") {
            ...
            let height = dim.width as f32;

            let mut scene = Scene::default();
            let mut grid_shader = GridShader::new();
            grid_shader.set_parameter_f32("grid_size", project.map.grid_size);
            grid_shader.set_parameter_f32("subdivisions", project.map.subdivisions);
            grid_shader.set_parameter_vec2(
                "offset",
                Vec2::new(project.map.offset.x, -project.map.offset.y),
            );
            scene.background = Some(Box::new(grid_shader));

            builder.build_entities_items(
                &project.map,
                &rusterix.assets,
                &mut scene,
            ...
            let translation_matrix = Mat3::<f32>::translation_2d(Vec2::new(
                project.map.offset.x + width / 2.0,
                -project.map.offset.y + height / 2.0,
            ));
            let scale_matrix = Mat3::new(
                project.map.grid_size,
                0.0,
                0.0,
                0.0,
                project.map.grid_size,
