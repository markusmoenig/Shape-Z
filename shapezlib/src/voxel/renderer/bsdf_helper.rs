use crate::prelude::*;
use rand::{Rng, rngs::ThreadRng};

const INV_PI: f32 = std::f32::consts::FRAC_1_PI;

#[inline]
pub fn lerp(a: f32, b: f32, t: f32) -> f32 {
    a * (1.0 - t) + b * t
}

#[inline]
pub fn reflect(i: Vec3<f32>, n: Vec3<f32>) -> Vec3<f32> {
    i - 2.0 * i.dot(n) * n
}

#[inline]
pub fn rsqrt(x: f32) -> f32 {
    1.0 / x.sqrt()
}

#[inline]
pub fn refract(i: Vec3<f32>, n: Vec3<f32>, eta: f32) -> Vec3<f32> {
    let cos_i = i.dot(n);
    let k = 1.0 - eta * eta * (1.0 - cos_i * cos_i);
    if k < 0.0 {
        Vec3::zero() // Total internal reflection
    } else {
        eta * i - (eta * cos_i + k.sqrt()) * n
    }
}

#[derive(PartialEq, Debug, Clone)]
pub enum MediumRole {
    None,
    Absorb,
    Scatter,
    Emissive,
}

// ---
#[derive(PartialEq, Debug, Clone)]
pub struct BSDFMedium {
    pub role: MediumRole,
    pub density: f32,
    pub color: Vec3<f32>,
    pub anisotropy: f32,
}

impl Default for BSDFMedium {
    fn default() -> Self {
        Self::new()
    }
}

impl BSDFMedium {
    pub fn new() -> Self {
        Self {
            role: MediumRole::None,
            density: 0.0,
            color: Vec3::zero(),
            anisotropy: 0.0,
        }
    }
}

#[derive(PartialEq, Debug, Clone)]
pub struct BSDFMaterial {
    pub base_color: Vec3<f32>,
    pub opacity: f32,
    pub alpha_mode: i32,
    pub alpha_cutoff: f32,
    pub emission: Vec3<f32>,
    pub anisotropic: f32,
    pub metallic: f32,
    pub roughness: f32,
    pub subsurface: f32,
    pub specular_tint: f32,
    pub sheen: f32,
    pub sheen_tint: f32,
    pub clearcoat: f32,
    pub clearcoat_roughness: f32,
    pub spec_trans: f32,
    pub ior: f32,
    pub ax: f32,
    pub ay: f32,
    pub medium: BSDFMedium,
}

impl Default for BSDFMaterial {
    fn default() -> Self {
        Self::new()
    }
}

impl BSDFMaterial {
    pub fn new() -> Self {
        Self {
            base_color: Vec3::new(0.5, 0.5, 0.5),
            opacity: 1.0,
            alpha_mode: 0,
            alpha_cutoff: 0.0,
            emission: Vec3::zero(),
            anisotropic: 0.0,
            metallic: 0.0,
            roughness: 0.5,
            subsurface: 0.0,
            specular_tint: 0.0,
            sheen: 0.0,
            sheen_tint: 0.0,
            clearcoat: 0.0,
            clearcoat_roughness: 0.0,
            spec_trans: 0.0,
            ior: 1.5,
            ax: 0.0,
            ay: 0.0,
            medium: BSDFMedium::new(),
        }
    }
    pub fn base_color_linear(&self) -> Vec3<F> {
        fn srgb_to_linear(c: F) -> F {
            if c <= 0.04045 {
                c / 12.92
            } else {
                ((c + 0.055) / 1.055).powf(2.4)
            }
        }

        Vec3::new(
            srgb_to_linear(self.base_color.x),
            srgb_to_linear(self.base_color.y),
            srgb_to_linear(self.base_color.z),
        )
    }
}

pub struct BSDFLight {
    pub position: Vec3<f32>,
    pub emission: Vec3<f32>,
    pub u: Vec3<f32>,
    pub v: Vec3<f32>,
    pub radius: f32,
    pub area: f32,
    pub type_: f32,
}

pub struct BSDFState {
    pub depth: i32,
    pub eta: f32,
    pub hit_dist: f32,

    pub fhp: Vec3<f32>,
    pub normal: Vec3<f32>,
    pub ffnormal: Vec3<f32>,
    pub tangent: Vec3<f32>,
    pub bitangent: Vec3<f32>,

    pub is_emitter: bool,

    pub tex_coord: Vec2<f32>,
    pub mat: BSDFMaterial,
    pub medium: BSDFMedium,
}

impl Default for BSDFState {
    fn default() -> Self {
        Self::new()
    }
}

impl BSDFState {
    pub fn new() -> Self {
        Self {
            depth: 0,
            eta: 0.0,
            hit_dist: 0.0,
            fhp: Vec3::zero(),
            normal: Vec3::zero(),
            ffnormal: Vec3::zero(),
            tangent: Vec3::zero(),
            bitangent: Vec3::zero(),
            is_emitter: false,
            tex_coord: Vec2::zero(),
            mat: BSDFMaterial::new(),
            medium: BSDFMedium::new(),
        }
    }
}

pub struct BSDFScatterSampleRec {
    pub l: Vec3<f32>,
    pub f: Vec3<f32>,
    pub pdf: f32,
}

impl Default for BSDFScatterSampleRec {
    fn default() -> Self {
        Self::new()
    }
}

impl BSDFScatterSampleRec {
    pub fn new() -> Self {
        Self {
            l: Vec3::zero(),
            f: Vec3::zero(),
            pdf: 0.0,
        }
    }
}

pub struct BSDFLightSampleRec {
    pub normal: Vec3<f32>,
    pub emission: Vec3<f32>,
    pub direction: Vec3<f32>,
    pub dist: f32,
    pub pdf: f32,
}

impl Default for BSDFLightSampleRec {
    fn default() -> Self {
        Self::new()
    }
}

impl BSDFLightSampleRec {
    pub fn new() -> Self {
        Self {
            normal: Vec3::zero(),
            emission: Vec3::zero(),
            direction: Vec3::zero(),
            dist: 0.0,
            pdf: 0.0,
        }
    }
}

pub fn face_forward(a: Vec3<f32>, b: Vec3<f32>) -> Vec3<f32> {
    if a.dot(b) < 0.0 { -b } else { b }
}

pub fn luminance(c: Vec3<f32>) -> f32 {
    0.212671 * c.x + 0.715160 * c.y + 0.072169 * c.z
}

pub fn gtr1(ndoth: f32, a: f32) -> f32 {
    if a >= 1.0 {
        return std::f32::consts::FRAC_1_PI;
    }
    let a2 = a * a;
    let t = 1.0 + (a2 - 1.0) * ndoth * ndoth;
    (a2 - 1.0) / (std::f32::consts::PI * a2.ln() * t)
}

pub fn sample_gtr1(rgh: f32, r1: f32, r2: f32) -> Vec3<f32> {
    let a = rgh.max(0.001);
    let a2 = a * a;

    let phi = r1 * std::f32::consts::TAU;

    let cos_theta = ((1.0 - a2.powf(1.0 - r2)) / (1.0 - a2)).sqrt();
    let sin_theta = (1.0 - (cos_theta * cos_theta)).sqrt().clamp(0.0, 1.0);
    let sin_phi = phi.sin();
    let cos_phi = phi.cos();

    Vec3::new(sin_theta * cos_phi, sin_theta * sin_phi, cos_theta)
}

pub fn gtr2(ndoth: f32, a: f32) -> f32 {
    let a2 = a * a;
    let t = 1.0 + (a2 - 1.0) * ndoth * ndoth;
    a2 / (std::f32::consts::PI * t * t)
}

pub fn sample_gtr2(rgh: f32, r1: f32, r2: f32) -> Vec3<f32> {
    let a = rgh.max(0.001);

    let phi = r1 * std::f32::consts::TAU;

    let cos_theta = ((1.0 - r2) / (1.0 + (a * a - 1.0) * r2)).sqrt();
    let sin_theta = (1.0 - (cos_theta * cos_theta)).sqrt().clamp(0.0, 1.0);
    let sin_phi = phi.sin();
    let cos_phi = phi.cos();

    Vec3::new(sin_theta * cos_phi, sin_theta * sin_phi, cos_theta)
}

pub fn sample_ggx_vndf(v: Vec3<f32>, ax: f32, ay: f32, r1: f32, r2: f32) -> Vec3<f32> {
    let vh = Vec3::new(ax * v.x, ay * v.y, v.z).normalized();

    let lensq = vh.x * vh.x + vh.y * vh.y;
    let tt1 = if lensq > 0.0 {
        Vec3::new(-vh.y, vh.x, 0.0) * rsqrt(lensq)
    } else {
        Vec3::new(1.0, 0.0, 0.0)
    };
    let tt2 = vh.cross(tt1);

    let r = r1.sqrt();
    let phi = 2.0 * std::f32::consts::PI * r2;
    let t1 = r * phi.cos();
    let t2 = r * phi.sin();
    let s = 0.5 * (1.0 + vh.z);
    let t2 = (1.0 - s) * (1.0 - t1 * t1).sqrt() + s * t2;

    let nh = t1 * tt1 + t2 * tt2 + (1.0 - t1 * t1 - t2 * t2).max(0.0).sqrt() * vh;

    Vec3::new(ax * nh.x, ay * nh.y, nh.z.max(0.0)).normalized()
}

pub fn gtr2_aniso(ndoth: f32, hdox: f32, hdoy: f32, ax: f32, ay: f32) -> f32 {
    let a = hdox / ax;
    let b = hdoy / ay;
    let c = a * a + b * b + ndoth * ndoth;
    1.0 / (std::f32::consts::PI * ax * ay * c * c)
}

pub fn sample_gtr2_aniso(ax: f32, ay: f32, r1: f32, r2: f32) -> Vec3<f32> {
    let phi = r1 * std::f32::consts::TAU;

    let sin_phi = ay * phi.sin();
    let cos_phi = ax * phi.cos();
    let tan_theta = (r2 / (1.0 - r2)).sqrt();

    Vec3::new(tan_theta * cos_phi, tan_theta * sin_phi, 1.0)
}

pub fn smith_g(ndotv: f32, alpha_g: f32) -> f32 {
    let a = alpha_g * alpha_g;
    let b = ndotv * ndotv;
    (2.0 * ndotv) / (ndotv + (a + b - a * b).sqrt())
}

pub fn smith_g_aniso(ndotv: f32, vdotx: f32, vdoty: f32, ax: f32, ay: f32) -> f32 {
    let a = vdotx * ax;
    let b = vdoty * ay;
    let c = ndotv;
    (2.0 * ndotv) / (ndotv + (a * a + b * b + c * c).sqrt())
}

pub fn schlick_weight(u: f32) -> f32 {
    let m = (1.0 - u).clamp(0.0, 1.0);
    let m2 = m * m;
    m2 * m2 * m
}

pub fn dielectric_fresnel(cos_theta_i: f32, eta: f32) -> f32 {
    let sin_theta_t_sq = eta * eta * (1.0 - cos_theta_i * cos_theta_i);

    if sin_theta_t_sq > 1.0 {
        return 1.0;
    }

    let cos_theta_t = (1.0 - sin_theta_t_sq).max(0.0).sqrt();

    let rs = (eta * cos_theta_t - cos_theta_i) / (eta * cos_theta_t + cos_theta_i);
    let rp = (eta * cos_theta_i - cos_theta_t) / (eta * cos_theta_i + cos_theta_t);

    0.5 * (rs * rs + rp * rp)
}

pub fn cosine_sample_hemisphere(r1: f32, r2: f32) -> Vec3<f32> {
    let r = r1.sqrt();
    let phi = std::f32::consts::TAU * r2;
    let x = r * phi.cos();
    let y = r * phi.sin();
    let z = (1.0 - x * x - y * y).max(0.0).sqrt();
    Vec3::new(x, y, z)
}

pub fn uniform_sample_hemisphere(r1: f32, r2: f32) -> Vec3<f32> {
    let r = (1.0 - r1 * r1).max(0.0).sqrt();
    let phi = std::f32::consts::TAU * r2;
    Vec3::new(r * phi.cos(), r * phi.sin(), r1)
}

pub fn uniform_sample_sphere(r1: f32, r2: f32) -> Vec3<f32> {
    let z = 1.0 - 2.0 * r1;
    let r = (1.0 - z * z).max(0.0).sqrt();
    let phi = std::f32::consts::TAU * r2;
    Vec3::new(r * phi.cos(), r * phi.sin(), z)
}

pub fn power_heuristic(a: f32, b: f32) -> f32 {
    let t = a * a;
    t / (b * b + t)
}

pub fn onb(n: Vec3<f32>, t: &mut Vec3<f32>, b: &mut Vec3<f32>) {
    let up = if n.z.abs() < 0.9999999 {
        Vec3::new(0.0, 0.0, 1.0)
    } else {
        Vec3::new(1.0, 0.0, 0.0)
    };
    *t = n.cross(up).normalized();
    *b = n.cross(*t);
}

pub fn sample_sphere_light(
    light: &BSDFLight,
    scatter_pos: Vec3<f32>,
    light_sample: &mut BSDFLightSampleRec,
    num_of_lights: i32,
    rng: &mut ThreadRng,
    max_distance: f32,
) {
    let r1 = rng.random();
    let r2 = rng.random();

    let mut sphere_center_to_surface = scatter_pos - light.position;
    let dist_to_sphere_center = sphere_center_to_surface.magnitude();

    sphere_center_to_surface /= dist_to_sphere_center;
    let sampled_dir = uniform_sample_hemisphere(r1, r2);
    let mut t = Vec3::zero();
    let mut b = Vec3::zero();
    onb(sphere_center_to_surface, &mut t, &mut b);
    let sampled_dir =
        t * sampled_dir.x + b * sampled_dir.y + sphere_center_to_surface * sampled_dir.z;

    let light_surface_pos = light.position + sampled_dir * light.radius;

    light_sample.direction = light_surface_pos - scatter_pos;
    light_sample.dist = light_sample.direction.magnitude();
    let dist_sq = light_sample.dist * light_sample.dist;

    light_sample.direction /= light_sample.dist;
    light_sample.normal = (light_surface_pos - light.position).normalized();
    light_sample.emission = light.emission * num_of_lights as f32;
    light_sample.pdf = dist_sq
        / (light.area * /*0.5*/ max_distance * light_sample.normal.dot(light_sample.direction).abs());
}

pub fn sample_rect_light(
    light: &BSDFLight,
    scatter_pos: Vec3<f32>,
    light_sample: &mut BSDFLightSampleRec,
    num_of_lights: i32,
    rng: &mut ThreadRng,
) {
    let r1: f32 = rng.random();
    let r2: f32 = rng.random();

    let light_surface_pos = light.position + light.u * r1 + light.v * r2;
    light_sample.direction = light_surface_pos - scatter_pos;
    light_sample.dist = light_sample.direction.magnitude();
    let dist_sq = light_sample.dist * light_sample.dist;
    light_sample.direction /= light_sample.dist;
    light_sample.normal = light.u.cross(light.v).normalized();
    light_sample.emission = light.emission * num_of_lights as f32;
    light_sample.pdf =
        dist_sq / (light.area * light_sample.normal.dot(light_sample.direction).abs());
}

pub fn sample_distant_light(
    light: &BSDFLight,
    scatter_pos: Vec3<f32>,
    light_sample: &mut BSDFLightSampleRec,
    num_of_lights: i32,
) {
    light_sample.direction = light.position;
    light_sample.normal = (scatter_pos - light.position).normalized();
    light_sample.emission = light.emission * num_of_lights as f32;
    light_sample.dist = f32::INFINITY;
    light_sample.pdf = 1.0;
}

// fn sample_one_light(light: &Light, scatter_pos: Vec3f, light_sample: &mut LightSampleRec) {
//     let light_type = light.type_ as i32;

//     if light_type == QUAD_LIGHT {
//         sample_rect_light(light, scatter_pos, light_sample);
//     } else if light_type == SPHERE_LIGHT {
//         sample_sphere_light(light, scatter_pos, light_sample);
//     } else {
//         sample_distant_light(light, scatter_pos, light_sample);
//     }
// }

pub fn sample_hg(v: Vec3<f32>, g: f32, r1: f32, r2: f32) -> Vec3<f32> {
    let cos_theta = if g.abs() < 0.001 {
        1.0 - 2.0 * r2
    } else {
        let sqr_term = (1.0 - g * g) / (1.0 + g - 2.0 * g * r2);
        -(1.0 + g * g - sqr_term * sqr_term) / (2.0 * g)
    };

    let phi = r1 * std::f32::consts::TAU;
    let sin_theta = (1.0 - (cos_theta * cos_theta)).sqrt().clamp(0.0, 1.0);
    let sin_phi = phi.sin();
    let cos_phi = phi.cos();

    let mut v1 = Vec3::zero();
    let mut v2 = Vec3::zero();
    onb(v, &mut v1, &mut v2);

    sin_theta * cos_phi * v1 + sin_theta * sin_phi * v2 + cos_theta * v
}

#[allow(clippy::excessive_precision)]
pub fn phase_hg(cos_theta: f32, g: f32) -> f32 {
    let denom = 1.0 + g * g + 2.0 * g * cos_theta;
    /*INV_4_PI*/
    0.07957747154594766 * (1.0 - g * g) / (denom * denom.sqrt())
}

pub fn to_world(x: Vec3<f32>, y: Vec3<f32>, z: Vec3<f32>, v: Vec3<f32>) -> Vec3<f32> {
    v.x * x + v.y * y + v.z * z
}

pub fn to_local(x: Vec3<f32>, y: Vec3<f32>, z: Vec3<f32>, v: Vec3<f32>) -> Vec3<f32> {
    Vec3::new(v.dot(x), v.dot(y), v.dot(z))
}

pub fn tint_colors(
    mat: &BSDFMaterial,
    eta: f32,
    f0: &mut f32,
    csheen: &mut Vec3<f32>,
    cspec0: &mut Vec3<f32>,
) {
    let lum = luminance(mat.base_color);
    let ctint = if lum > 0.0 {
        mat.base_color / lum
    } else {
        Vec3::new(1.0, 1.0, 1.0)
    };

    *f0 = (1.0 - eta) / (1.0 + eta);
    *f0 *= *f0;

    *cspec0 = *f0 * Vec3::lerp(Vec3::new(1.0, 1.0, 1.0), ctint, mat.specular_tint);
    *csheen = Vec3::lerp(Vec3::new(1.0, 1.0, 1.0), ctint, mat.sheen_tint);
}

pub fn eval_disney_diffuse(
    mat: &BSDFMaterial,
    csheen: Vec3<f32>,
    v: Vec3<f32>,
    l: Vec3<f32>,
    h: Vec3<f32>,
    pdf: &mut f32,
) -> Vec3<f32> {
    *pdf = 0.0;
    if l.z <= 0.0 {
        return Vec3::zero();
    }

    let l_dot_h = l.dot(h);

    let rr = 2.0 * mat.roughness * l_dot_h * l_dot_h;

    // Diffuse
    let fl = schlick_weight(l.z);
    let fv = schlick_weight(v.z);
    let fretro = rr * (fl + fv + fl * fv * (rr - 1.0));
    let fd = (1.0 - 0.5 * fl) * (1.0 - 0.5 * fv);

    // Fake subsurface
    let fss90 = 0.5 * rr;
    let fss = lerp(1.0, fss90, fl) * lerp(1.0, fss90, fv);
    let ss = 1.25 * (fss * (1.0 / (l.z + v.z) - 0.5) + 0.5);

    // Sheen
    let fh = schlick_weight(l_dot_h);
    let fsheen = fh * mat.sheen * csheen;

    *pdf = l.z * INV_PI;
    INV_PI * mat.base_color * lerp(fd + fretro, ss, mat.subsurface) + fsheen
}

pub fn eval_microfacet_reflection(
    mat: &BSDFMaterial,
    v: Vec3<f32>,
    l: Vec3<f32>,
    h: Vec3<f32>,
    f: Vec3<f32>,
    pdf: &mut f32,
) -> Vec3<f32> {
    *pdf = 0.0;
    if l.z <= 0.0 {
        return Vec3::zero();
    }

    let d = gtr2_aniso(h.z, h.x, h.y, mat.ax, mat.ay);
    let g1 = smith_g_aniso(v.z.abs(), v.x, v.y, mat.ax, mat.ay);
    let g2 = g1 * smith_g_aniso(l.z.abs(), l.x, l.y, mat.ax, mat.ay);

    *pdf = g1 * d / (4.0 * v.z);
    f * d * g2 / (4.0 * l.z * v.z)
}

pub fn eval_microfacet_refraction(
    mat: &BSDFMaterial,
    eta: f32,
    v: Vec3<f32>,
    l: Vec3<f32>,
    h: Vec3<f32>,
    f: Vec3<f32>,
    pdf: &mut f32,
) -> Vec3<f32> {
    *pdf = 0.0;
    if l.z >= 0.0 {
        return Vec3::zero();
    }

    let l_dot_h = l.dot(h);
    let v_dot_h = v.dot(h);

    let d = gtr2_aniso(h.z, h.x, h.y, mat.ax, mat.ay);
    let g1 = smith_g_aniso(v.z.abs(), v.x, v.y, mat.ax, mat.ay);
    let g2 = g1 * smith_g_aniso(l.z.abs(), l.x, l.y, mat.ax, mat.ay);
    let denom = l_dot_h + v_dot_h * eta;
    let denom = denom * denom;
    let eta2 = eta * eta;
    let jacobian = l_dot_h.abs() / denom;

    *pdf = g1 * v_dot_h.max(0.0) * d * jacobian / v.z;
    // powf(mat.base_color, 0.5) * (1.0 - f) * d * g2 * v_dot_h.abs() * jacobian * eta2
    //     / (l.z * v.z).abs()

    mat.base_color
        .map(|c| c.powf(0.5))      // same as powf(base_color, 0.5)
        * (Vec3::one() - f)
        * d * g2 * v_dot_h.abs() * jacobian * eta2
        / (l.z * v.z).abs()
}

pub fn eval_clearcoat(
    mat: &BSDFMaterial,
    v: Vec3<f32>,
    l: Vec3<f32>,
    h: Vec3<f32>,
    pdf: &mut f32,
) -> Vec3<f32> {
    *pdf = 0.0;
    if l.z <= 0.0 {
        return Vec3::zero();
    }

    let v_dot_h = v.dot(h);

    let f = lerp(0.04, 1.0, schlick_weight(v_dot_h));
    let d = gtr1(h.z, mat.clearcoat_roughness);
    let g = smith_g(l.z, 0.25) * smith_g(v.z, 0.25);
    let jacobian = 1.0 / (4.0 * v_dot_h);

    *pdf = d * h.z * jacobian;
    Vec3::new(f, f, f) * d * g
}

pub fn disney_sample(
    state: &BSDFState,
    v: Vec3<f32>,
    n: Vec3<f32>,
    ll: &mut Vec3<f32>,
    pdf: &mut f32,
    rng: &mut ThreadRng,
) -> Vec3<f32> {
    *pdf = 0.0;

    let r1 = rng.random();
    let r2 = rng.random();

    // TODO: Tangent and bitangent should be calculated from mesh (provided, the mesh has proper uvs)
    let mut t = Vec3::zero();
    let mut b = Vec3::zero();
    onb(n, &mut t, &mut b);

    // Transform to shading space to simplify operations (NDotL = L.z; NDotV = V.z; NDotH = H.z)
    let v = to_local(t, b, n, v);

    // Tint colors
    let mut csheen = Vec3::zero();
    let mut cspec0 = Vec3::zero();
    let mut f0 = 0.0;
    tint_colors(&state.mat, state.eta, &mut f0, &mut csheen, &mut cspec0);

    // Model weights
    let dielectric_wt = (1.0 - state.mat.metallic) * (1.0 - state.mat.spec_trans);
    let metal_wt = state.mat.metallic;
    let glass_wt = (1.0 - state.mat.metallic) * state.mat.spec_trans;

    // Lobe probabilities
    let schlick_wt = schlick_weight(v.z);

    let diff_pr = dielectric_wt * luminance(state.mat.base_color);
    let dielectric_pr =
        dielectric_wt * luminance(Vec3::lerp(cspec0, Vec3::new(1.0, 1.0, 1.0), schlick_wt));
    let metal_pr = metal_wt
        * luminance(Vec3::lerp(
            state.mat.base_color,
            Vec3::new(1.0, 1.0, 1.0),
            schlick_wt,
        ));
    let glass_pr = glass_wt;
    let clear_ct_pr = 0.25 * state.mat.clearcoat;

    // Normalize probabilities
    let inv_total_wt = 1.0 / (diff_pr + dielectric_pr + metal_pr + glass_pr + clear_ct_pr);
    let diff_pr = diff_pr * inv_total_wt;
    let dielectric_pr = dielectric_pr * inv_total_wt;
    let metal_pr = metal_pr * inv_total_wt;
    let glass_pr = glass_pr * inv_total_wt;
    let clear_ct_pr = clear_ct_pr * inv_total_wt;

    // CDF of the sampling probabilities
    let cdf = [
        diff_pr,
        diff_pr + dielectric_pr,
        diff_pr + dielectric_pr + metal_pr,
        diff_pr + dielectric_pr + metal_pr + glass_pr,
        diff_pr + dielectric_pr + metal_pr + glass_pr + clear_ct_pr,
    ];

    // Sample a lobe based on its importance
    let r3: f32 = rng.random();

    let l = if r3 < cdf[0] {
        cosine_sample_hemisphere(r1, r2)
    } else if r3 < cdf[2] {
        let mut h = sample_ggx_vndf(v, state.mat.ax, state.mat.ay, r1, r2);

        if h.z < 0.0 {
            h = -h;
        }

        reflect(-v, h).normalized()
    } else if r3 < cdf[3] {
        let mut h = sample_ggx_vndf(v, state.mat.ax, state.mat.ay, r1, r2);
        let f = dielectric_fresnel(v.dot(h).abs(), state.eta);

        if h.z < 0.0 {
            h = -h;
        }

        let r3 = (r3 - cdf[2]) / (cdf[3] - cdf[2]);

        if r3 < f {
            reflect(-v, h).normalized()
        } else {
            refract(-v, h, state.eta).normalized()
        }
    } else {
        let mut h = sample_gtr1(state.mat.clearcoat_roughness, r1, r2);

        if h.z < 0.0 {
            h = -h;
        }

        reflect(-v, h).normalized()
    };

    let l = to_world(t, b, n, l);
    let v = to_world(t, b, n, v);

    *ll = l;

    disney_eval(state, v, n, l, pdf)
}

pub fn disney_eval(
    state: &BSDFState,
    vv: Vec3<f32>,
    nn: Vec3<f32>,
    ll: Vec3<f32>,
    pdf: &mut f32,
) -> Vec3<f32> {
    *pdf = 0.0;
    let mut f = Vec3::zero();

    // TODO: Tangent and bitangent should be calculated from mesh (provided, the mesh has proper uvs)
    let mut t = Vec3::zero();
    let mut b = Vec3::zero();
    onb(nn, &mut t, &mut b);

    // Transform to shading space to simplify operations (NDotL = L.z; NDotV = V.z; NDotH = H.z)
    let v = to_local(t, b, nn, vv);
    let l = to_local(t, b, nn, ll);

    let mut h = if l.z > 0.0 {
        (l + v).normalized()
    } else {
        (l + v * state.eta).normalized()
    };

    if h.z < 0.0 {
        h = -h;
    }

    // Tint colors
    let mut csheen = Vec3::zero();
    let mut cspec0 = Vec3::zero();
    let mut f0 = 0.0;
    tint_colors(&state.mat, state.eta, &mut f0, &mut csheen, &mut cspec0);

    // Model weights
    let dielectric_wt = (1.0 - state.mat.metallic) * (1.0 - state.mat.spec_trans);
    let metal_wt = state.mat.metallic;
    let glass_wt = (1.0 - state.mat.metallic) * state.mat.spec_trans;

    // Lobe probabilities
    let schlick_wt = schlick_weight(v.z);

    let diff_pr = dielectric_wt * luminance(state.mat.base_color);
    let dielectric_pr =
        dielectric_wt * luminance(Vec3::lerp(cspec0, Vec3::new(1.0, 1.0, 1.0), schlick_wt));
    let metal_pr = metal_wt
        * luminance(Vec3::lerp(
            state.mat.base_color,
            Vec3::new(1.0, 1.0, 1.0),
            schlick_wt,
        ));
    let glass_pr = glass_wt;
    let clear_ct_pr = 0.25 * state.mat.clearcoat;

    // Normalize probabilities
    let inv_total_wt = 1.0 / (diff_pr + dielectric_pr + metal_pr + glass_pr + clear_ct_pr);
    let diff_pr = diff_pr * inv_total_wt;
    let dielectric_pr = dielectric_pr * inv_total_wt;
    let metal_pr = metal_pr * inv_total_wt;
    let glass_pr = glass_pr * inv_total_wt;
    let clear_ct_pr = clear_ct_pr * inv_total_wt;

    let reflect = l.z * v.z > 0.0;

    let mut tmp_pdf = 0.0;
    let v_dot_h = v.dot(h).abs();

    if diff_pr > 0.0 && reflect {
        f += eval_disney_diffuse(&state.mat, csheen, v, l, h, &mut tmp_pdf) * dielectric_wt;
        *pdf += tmp_pdf * diff_pr;
    }

    if dielectric_pr > 0.0 && reflect {
        let ff = (dielectric_fresnel(v_dot_h, 1.0 / state.mat.ior) - f0) / (1.0 - f0);

        f += eval_microfacet_reflection(
            &state.mat,
            v,
            l,
            h,
            Vec3::lerp(cspec0, Vec3::new(1.0, 1.0, 1.0), ff),
            &mut tmp_pdf,
        ) * dielectric_wt;
        *pdf += tmp_pdf * dielectric_pr;
    }

    if metal_pr > 0.0 && reflect {
        let ff = Vec3::lerp(
            state.mat.base_color,
            Vec3::new(1.0, 1.0, 1.0),
            schlick_weight(v_dot_h),
        );

        f += eval_microfacet_reflection(&state.mat, v, l, h, ff, &mut tmp_pdf) * metal_wt;
        *pdf += tmp_pdf * metal_pr;
    }

    if glass_pr > 0.0 {
        let ff = dielectric_fresnel(v_dot_h, state.eta);

        if reflect {
            f += eval_microfacet_reflection(
                &state.mat,
                v,
                l,
                h,
                Vec3::new(ff, ff, ff),
                &mut tmp_pdf,
            ) * glass_wt;
            *pdf += tmp_pdf * glass_pr * ff;
        } else {
            f += eval_microfacet_refraction(
                &state.mat,
                state.eta,
                v,
                l,
                h,
                Vec3::new(ff, ff, ff),
                &mut tmp_pdf,
            ) * glass_wt;
            *pdf += tmp_pdf * glass_pr * (1.0 - ff);
        }
    }

    if clear_ct_pr > 0.0 && reflect {
        f += eval_clearcoat(&state.mat, v, l, h, &mut tmp_pdf) * 0.25 * state.mat.clearcoat;
        *pdf += tmp_pdf * clear_ct_pr;
    }

    f * l.z.abs()
}
